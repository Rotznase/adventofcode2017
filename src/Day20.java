...
public class Day20 extends AdventOfCode {
    ...
    void run() {
        ...
        while (count < 1000) {
            ...
            final long shortestDistance = ensemble.stream().mapToLong(P::distance).min().orElse(Long.MAX_VALUE);
            final List<P> pointWithShortestDistance = ensemble.stream().filter(p -> p.distance() == shortestDistance).collect(Collectors.toList());

            candidateNew = pointWithShortestDistance.stream().findFirst().orElse(null);

            if (candidateOld != null) {
                if (candidateNew.distance() <= candidateOld.distance()) {
                    if (candidateNew == candidateOld) {
                        count++;
                    }
                    candidateOld = candidateNew;
                }
            } else {
                candidateOld = candidateNew;
            }
    ...
    private void parseInput(Collection<P> col, String[] input) {
        Pattern pattern = Pattern.compile("(([pva])=<(.*?)>)");
        for (String line: input) {
            ...
            while (m.find()) {
                ...
                String[] coords = m.group(3).split(",");
                switch (letter) {
                    case "p": p.setP(parseLong(coords[0].trim()), parseLong(coords[1].trim()), parseLong(coords[2].trim())); break;
                    case "v": p.setV(parseLong(coords[0].trim()), parseLong(coords[1].trim()), parseLong(coords[2].trim())); break;
                    case "a": p.setA(parseLong(coords[0].trim()), parseLong(coords[1].trim()), parseLong(coords[2].trim())); break;
                }
            }
        }
    }
    ...
    private class P {
        ...
        void setV(long x, long y, long z) {
            vx = x;
            vy = y;
            vz = z;
        }

        void setA(long x, long y, long z) {
            ax = x;
            ay = y;
            az = z;
        }
        ...
        public boolean equals(Object o) {
            ...
            if (px != p.px) return false;
            if (py != p.py) return false;
            if (pz != p.pz) return false;
            if (vx != p.vx) return false;
            if (vy != p.vy) return false;
            if (vz != p.vz) return false;
            if (ax != p.ax) return false;
            if (ay != p.ay) return false;
            return az == p.az;
        }

        @Override
        public int hashCode() {
            int result = (int) (px ^ (px >>> 32));
            result = 31 * result + (int) (py ^ (py >>> 32));
            result = 31 * result + (int) (pz ^ (pz >>> 32));
            result = 31 * result + (int) (vx ^ (vx >>> 32));
            result = 31 * result + (int) (vy ^ (vy >>> 32));
            result = 31 * result + (int) (vz ^ (vz >>> 32));
            result = 31 * result + (int) (ax ^ (ax >>> 32));
            result = 31 * result + (int) (ay ^ (ay >>> 32));
            result = 31 * result + (int) (az ^ (az >>> 32));
            return result;
        }
    }
