...
public class Day07 extends AdventOfCode {
    ...
    public void run() {
        ...
        Map<String, Program> progList = new HashMap();

        Pattern programName = Pattern.compile("(\\w+) \\((\\d+)\\)(.*)");
        for (String entry: input) {
            final Matcher matcher = programName.matcher(entry);
            if (matcher.find()) {
                final String name = matcher.group(1);
                final int nr = Integer.parseInt(matcher.group(2));
                Program prog = new Program(name, nr);
                progList.put(name, prog);
            }
        }

        Pattern children = Pattern.compile("(\\w+) \\((\\d+)\\) -> (.*)");
        for (String entry: input) {
            final Matcher matcher = children.matcher(entry);
            if (matcher.find()) {
                String name = matcher.group(1);
                Program program = progList.get(name);

                final String[] subProgNames = matcher.group(3).split(",\\s*");
                for (String subName: subProgNames) {
                    Program subProg = progList.get(subName);
                    program.addSubProg(subProg);
                }
            }
        }

        Program root = Iterables.find(progList.values(), new Predicate<Program>() {
            @Override
            public boolean apply(final Program input) {
                return input.isRoot();
            }
        });
        ...
        Set<Integer> x = FluentIterable.from(progList.values())
                .filter(new Predicate<Program>() {
                    @Override
                    public boolean apply(final Program input) {
                        return !input.isBalanced();
                    }
                })
                .transform(new Function<Program, Integer>() {
                    @Override
                    public Integer apply(final Program input) {
                        return input.weight();
                    }
                })
                .toSet();
        final Integer min = Collections.min(x);

        Program unbalancedCandidate = Iterables.find(progList.values(), new Predicate<Program>() {
            @Override
            public boolean apply(final Program input) {
                return input.weight() == min;
            }
        });

        for (Program ch: unbalancedCandidate.subProgs)
            System.out.println(ch.name+" "+ch.nr+" "+ch.weight());
    ...
    private static class Program {
        ...
        int nr;
        Program parent;
        List<Program> subProgs;

        Program(final String name, final int nr) {
            this.name = name;
            this.nr = nr;
            this.parent = null;
        ...
        void addSubProg(Program subProg) {
            subProgs.add(subProg);
            subProg.parent = this;
        }
